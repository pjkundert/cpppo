...
# Creation Factories
#---------------------------------------------------------------------------#
def StartTcpServerLogging(context=None, identity=None, framer=ModbusSocketFramer, port=502):
...
for port in xrange( address[1], address[1] + int( args.range )):
    ...
    try:
        StartTcpServerLogging( context, framer=framer, port = port)
