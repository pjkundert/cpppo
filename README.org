...
*** EtherNet/IP Controller Client
    ...
    Send List Identity/Services requests:
    : -i|--list-identity
    : -s|--list-services
    ...
    #+BEGIN_EXAMPLE
    $ python -m cpppo.server.enip.client --udp --broadcast  --list-identity -v -a
        255.255.255.255
    List Identity  0 from ('10.0.0.4', 44818): {
        "sin_addr": "10.0.0.4",
        "status_word": 12640,
        "vendor_id": 1,
        ...
        "product_code": 54,
        "product_revision": 2836
    }
    List Identity  1 from ('192.168.0.201', 44818): {
        "sin_addr": "192.168.0.201",
        "status_word": 12640,
        "vendor_id": 1,
        "product_name": "1756-L61/C LOGIX5561",
        "sin_port": 44818,
        "state": 255,
        "version": 1,
        "device_type": 14,
...
**** =cpppo.server.enip= =client.connector= class
     ...
     Identity or List Services requests.  If broadcast (and a "broadcast" IP
     address such as 255.255.255.255 is used), then multiple responses should be
     expected; the default =cpppo.server.enip.client= module entrypoint will
     wait for the full duration of the specified =timeout= for responses.
...
**** =client.connector.list_identity= and =.list_services=

     These methods issue List Identity and List Services requests, valid on
     either UDP/IP or TCP/IP connections (or via UDP/IP broadcast).  The
     response(s) may be harvested by awaiting for incoming activity on the
     socket:
     #+BEGIN_EXAMPLE python
     ...
     optional arguments:
       ...
       -b, --broadcast       Allow multiple peers, and use of broadcast address
                             (default: False)
       --identity            List Identity (default: False)
       --interfaces          List Interfaces (default: False)
     ...
     content of each EtherNet/IP CIP response is printed, in JSON format.

     To obtain responses from all EtherNet/IP CIP devices on the local LAN:
     #+BEGIN_EXAMPLE
     $ python -m cpppo.server.enip.list_services --identity --udp --broadcast -a 255.255.255.255
     {
         "peer": [
             "10.0.0.4",
             44818
...
*** EtherNet/IP =cpppo.server.enip.getattr= API

    Many devices such as Rockwell MicroLogix, Allen-Bradley PowerFlex that
...
**** =cpppo.server.enip= =getattr.proxy[_simple]= classes
    ...
    device, such as an A-B PowerFlex, with a CIP =REAL= attribute at Class
    0x93, Instance 1, Attribute 10:
    #+BEGIN_EXAMPLE python
    ...
    Then, in your Python code, to access the parameter from the remote device:
    #+BEGIN_EXAMPLE python
    ...
    1. You can =.read= 1 or more values.  Here, we supply a single Python =str=,
       so the =proxy= deduces that you want one named parameter value.
    2. The =.read= returns a sequence of all the requested values, so we use
       Python =tuple= assignment to unpack a sequence containing a single value:
       : variable, = [value]
