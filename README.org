...
*** EtherNet/IP Controller Client
    ...
    The default request timeout is specified in "timeout_ticks".  Each tick has a duration from 1 to
    32,768 milliseconds, computed in ms.: =2 ** <priority_tick_time>=
    : --priority-time-tick 0                            # 1ms. ticks
