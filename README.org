...
** Vagrant
   ...
   : $ make vmware-up # or virtualbox-up
...
** Docker
   ...
   : vagrant@precise64:~$ docker run -p 44818:44818 -d cpppo/cpppo python -m cpppo.server.enip SCADA=dint[1000]

   Assuming you have cpppo installed on your local host, you can now test this.
   We'll read SCADA[1] and SCADA[0-10], repeating 10 times:
...
*** Creating Docker images from a Dockerfile
    ...
    : vagrant@precise64:~$ cd /src/cpppo/docker/cpppo/cpppo
    : vagrant@precise64:~$ docker build -t cpppo/cpppo .
