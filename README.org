...
*** EtherNet/IP Controller Client
    ...
    processing Unconnected requests via TPC/IP or UDP/IP sessions to CIP devices
    -- either Controllers (eg. Rockwell ControlLogix, CompactLogix) which can
    "route" CIP requests, or the =enip_get_attribute -S ...= command for access
    to simple CIP devices (eg. Rockwell MicroLogix, A-B PowerFlex, ...) which
    can only perform basic CIP services such as "Get Attribute Single/All", and
    do not understand the "routing" CIP Unconnected Send encapsulation required
    by the more advanced "routing" Controllers.
    
    These commands can Register and issue a stream of "Unconnected" requests to
    the Controller, such as Get/Set Attribute or *Logix Read/Write Tag
    (optionally Fragmented) requests.

    For Read/Write Tag [Fragmented] requests, by default to a "routing" device
    (eg. ControlLogix, CompactLogix):
    ...
    To use Get Attribute Single/All requests (suitable for simpler devices,
    usually also used with the =-S= option), use these commands instead (use the
    =--help= option to see their options, which are quite similar to
    =cpppo.server.enip.client= and =enip_client=):
    : $ python -m cpppo.server.enip.get_attribute -S ...
    : $ enip_get_attribute -S ...
...
**** =client.connector.get_attribute_single= and =.get_attributes_all=
     ...
     from =cpppo/server/enip/get_attribute.py=:
     #+BEGIN_EXAMPLE python
