...
*** Building a Vagrant Image
    ...
    Vagrantfiles are hosted at http://box.hardconsulting.com.  The default box
    in cpppo/vagrant/debian/Virtualbox is a VMware image: .../jessie64.box;
    change this to reference the .../jessie64-virtualbox.box if you decide to use
    the VirtualBox VM to host your Debian image.
    
    Using the veewee tool, build a VirtualBox (or VMware) image.  This downloads
    the Debian installer image and VirtualBox Guest Additions, runs it (you may
    need to watch the VirtualBox GUI, and help it complete the final Grub
    installation on /dev/sda), and then packages up the VM as a Vagrant box:
    ...
    : $ vagrant box add jessie64 Debian-testing-amd64-netboot.box --provider virtualbox
