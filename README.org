...
*** EtherNet/IP Controller Object Configuration
    ...
    =/etc= or =%APPDATA%=, a =.cpppo.cfg= in your home directory, or a
    =cpppo.cfg= file in the current working directory where your application is
    run.
    ...
    Use the new EtherNet/IP CIP =cpppo.server.enip.poll= API to see the results
...
*** EtherNet/IP =cpppo.server.enip.poll= API
    ...
    #+BEGIN_EXAMPLE
    $ cpppo -m cpppo.server.enip.poll_test.py
    #+END_EXAMPLE

    and then start the (above) =poll_example.py=:
    #+BEGIN_EXAMPLE
    $ cpppo -m cpppo.server.enip.poll_example.py
    #+END_EXAMPLE
    
    you'll see something like this (make sure you stop/pause and then restart
    the =poll_test.py= A-B PowerFlex simulator during the test):
    #+BEGIN_EXAMPLE
